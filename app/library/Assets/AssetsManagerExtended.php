...
class AssetsManagerExtended extends Manager
{
    ...
    $logger->error((string)__LINE__. ' - '. $completeTargetPath);
            ...
            if ($join) {
                ...
                if (!$completeTargetPath) {
                    throw new \Phalcon\Exception("Path '". $completeTargetPath. "' is not a valid target path (1)");
                }

                if (is_dir($completeTargetPath)) {
                    throw new \Phalcon\Exception("Path '". $completeTargetPath. "' is not a valid target path (2), is dir.");
                }
            }
        }
    ...
    $logger->error((string)__LINE__. ' - '. $sourcePath);
                    ...
                    if (!$sourcePath) {
                        ...
                        throw new \Phalcon\Exception("Resource '". $sourcePath. "' does not have a valid source path");
                    }
                } else {
    ...
    $logger->error((string)__LINE__. ' - '. $targetPath);
                /** We need a valid final target path */
                if (!$targetPath) {
                    throw new \Phalcon\Exception("Resource '". $sourcePath. "' does not have a valid target path");
                }
                ...
                if ($local) {
                    ...
                    if ($targetPath == $sourcePath) {
                        throw new \Phalcon\Exception("Resource '". $targetPath. "' have the same source and target paths");
                    }
    ...
    $logger->error((string)__LINE__ . ' - '. (string)($filter instanceof \Phalcon\Assets\FilterInterface));
    $logger->error((string)__LINE__ . ' - '. gettype($filter));
                        /** Filters must be valid objects */
//                        if (gettype($filter) != "object") {
                        if (!($filter instanceof \Phalcon\Assets\FilterInterface)) {
    $logger->error((string)__LINE__ . "Filter is invalid");
                            throw new \Phalcon\Exception("Filter is invalid");
                        }
    ...
    $logger->error((string)__LINE__);
                    ...
                    if ($join == true) {
                        if ($type == $typeCss) {
    $logger->error((string)__LINE__ . ' - '. $type);
    ...
    $logger->error((string)__LINE__. ' - '. $local);
        ...
        $logger->error((string)__LINE__. ' - '. "DONE");
        return $output;
    }
}
